    ...
    <div className="space-y-6">
      ...
      {walletArray.length > 0 && (
        <div>
          ...
          {/* Protocol Info */}
          <div className="p-4 mt-4 bg-white rounded shadow">
            <h3 className="mb-2 text-lg font-medium">Protocol Statistics</h3>
            
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              <div>
                <h4 className="mb-2 text-sm font-semibold">Detected Protocols</h4>
                <ul className="pl-5 list-disc">
                  {Array.from(new Set(
                    connectionList
                      .filter(conn => conn.protocolName)
                      .map(conn => conn.protocolName)
                  )).map(protocolName => (
                    <li key={protocolName}>{protocolName}</li>
                  ))}
                  {connectionList.filter(conn => conn.protocolName).length === 0 && (
                    <li className="text-gray-500">No protocols detected yet</li>
                  )}
                </ul>
              </div>
              
              <div>
                <h4 className="mb-2 text-sm font-semibold">Protocol Distribution</h4>
                {connectionList.length > 0 ? (
                  <div className="p-2 bg-gray-100 rounded">
                    {/* Protocol distribution stats - basic implementation */}
                    {Array.from(
                      connectionList.reduce((acc, conn) => {
                        const category = conn.category || 'unknown';
                      ...
                      <div key={category} className="flex items-center mb-1">
                        ...
                        <div className="flex-1">
                          <div 
                            ...
                            style={{ 
                              width: `${Math.min(100, (count / connectionList.length) * 100)}%`,
                              backgroundColor: getCategoryColor(category)
                            }}
                          ></div>
                        </div>
                        <div className="w-12 ml-2 text-xs text-right">
                          {Math.round((count / connectionList.length) * 100)}%
                        </div>
                      </div>
                    ))}
                  </div>
...
// Helper function to get color for protocol category
